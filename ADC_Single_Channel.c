...
void readEVADC(void)
{
    Ifx_EVADC_G_RES conversionResult;                               /* Wait for valid result */
    do
    {
        conversionResult = IfxEvadc_Adc_getResult(&g_adcChannel);   /* Read the result of the channel */
    } while(!conversionResult.B.VF);

    g_result = conversionResult;                                    /* Store result */
}
...
{
    sprintf(uartBuff, "ADC Value : %d\n\r", g_result.B.RESULT);
    send_UART(uartBuff);                                            /*Task 1-1 printing ADC value through UART*/
    adc_p = ((float)g_result.B.RESULT / 4096) * 100;
    sprintf(adcpBuff, "ADC Percentage : %.2f %\n\r", adc_p);
    send_UART(adcpBuff);                                            /*Task 1-2 printing ADC value in percentage(UART)*/

    if(g_result.B.RESULT <= LIMIT_LOW)                              /*Task 1-3 LED 13.1 indicates if the value
                                                                      is less than or equal to the set limit 0x555 */
    {

...
void init_UART(void)
{
    ...
    ascConfig.interrupt.typeOfService = IfxCpu_Irq_getTos(IfxCpu_Index_0);
...
{
    uint32 string[20];
    sprintf(string,"123456");                                  /* Message to send                          */
    Ifx_SizeT count = strlen(string);                                               /* Size of the message                      */
    IfxAsclin_Asc_write(&g_asc, string, &count, TIME_INFINITE);         /* Transfer of data                         */
}
void send_UART(char *ptr)
{                                 /* Message to send                          */
    Ifx_SizeT count = strlen(ptr);                                               /* Size of the message                      */
    IfxAsclin_Asc_write(&g_asc, ptr, &count, TIME_INFINITE);         /* Transfer of data                         */
}
